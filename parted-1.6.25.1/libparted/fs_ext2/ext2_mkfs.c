				...
				struct ext2_group_desc *gd)
{
	...
	numgroups = howmany(EXT2_SUPER_BLOCKS_COUNT(*sb)
				- EXT2_SUPER_FIRST_DATA_BLOCK(*sb),
			    EXT2_SUPER_BLOCKS_PER_GROUP(*sb));
	gdblocks = howmany(numgroups * sizeof(struct ext2_group_desc),
				...
				PedTimer* timer)
{
	...
	numgroups = howmany(EXT2_SUPER_BLOCKS_COUNT(*sb)
				- EXT2_SUPER_FIRST_DATA_BLOCK(*sb),
			    EXT2_SUPER_BLOCKS_PER_GROUP(*sb));
	itsize = howmany(sizeof(struct ext2_inode)
			      ...
			      int reserved_block_percentage)
{
	...
	sb->s_r_blocks_count = PED_CPU_TO_LE32(((u_int64_t)numblocks
			  ...
			  PedTimer* timer)
{
	...
	sparse_admin = non_sparse_admin
		       + howmany (numgroups * sizeof(struct ext2_group_desc),
				  1 << log_block_size);
