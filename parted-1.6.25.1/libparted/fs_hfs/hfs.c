...
hfsplus_open (PedGeometry* geom)
{
	...
	if (priv_data->attributes_file->sect_nb != 0) {
		if (ped_exception_throw (
			...
			"I haven't seen used anywhere.  Parted can "
			"theoretically handle it, but the corresponding code "
			"has never been tested, so this might be risky.  "
			"Please email me so I can see how it works!  "
			"<xxx@yyy.zz>"))
				!= PED_EXCEPTION_IGNORE)
			goto hpo_cl;
	}
