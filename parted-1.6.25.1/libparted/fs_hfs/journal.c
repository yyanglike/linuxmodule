...
hfsj_update_jl(PedFileSystem* fs, uint32_t block)
{
	...
	binsect = PED_BE32_TO_CPU(priv_data->vh->block_size) / PED_SECTOR_SIZE;
	...
	offset = (uint64_t)block * PED_SECTOR_SIZE * binsect;
		  ...
		  void* buf)
{
	...
	while (nb_sect--) {
		...
		buf = ((uint8_t*)buf) + PED_SECTOR_SIZE;
		read_sect++;
			...
			PedSector jsector, PedSector jlength)
{
	...
	blhdr_nbsect = PED_BE32_TO_CPU(jh->blhdr_size) / PED_SECTOR_SIZE;
	blhdr = (HfsJBlockListHeader*)
		  ped_malloc (blhdr_nbsect * PED_SECTOR_SIZE);
	if (!blhdr) return 0;

	start = PED_BE64_TO_CPU(jh->start) / PED_SECTOR_SIZE;
	do {
	...
	jh->start = PED_CPU_TO_BE64(start * PED_SECTOR_SIZE);
...
hfsj_replay_journal(PedFileSystem* fs)
{
	...
	if (   (PED_BE64_TO_CPU(jib->offset) % PED_SECTOR_SIZE)
	    || (PED_BE64_TO_CPU(jib->size)   % PED_SECTOR_SIZE) ) {
		ped_exception_throw (
			PED_EXCEPTION_NO_FEATURE,
			PED_EXCEPTION_CANCEL,
	...
	sector = PED_BE64_TO_CPU(jib->offset) / PED_SECTOR_SIZE;
	length = PED_BE64_TO_CPU(jib->size)   / PED_SECTOR_SIZE;
	...
	if ( (PED_BE64_TO_CPU(jh->size)%PED_SECTOR_SIZE)
	  || (PED_BE64_TO_CPU(jh->size)/PED_SECTOR_SIZE != (uint64_t)length) ) {
		ped_exception_throw (
			PED_EXCEPTION_ERROR,
			PED_EXCEPTION_CANCEL,
	...
	if (   (PED_BE64_TO_CPU(jh->start) % PED_SECTOR_SIZE)
	    || (PED_BE64_TO_CPU(jh->end)   % PED_SECTOR_SIZE)
	    || (PED_BE32_TO_CPU(jh->blhdr_size) % PED_SECTOR_SIZE)
	    || (PED_BE32_TO_CPU(jh->jhdr_size)  % PED_SECTOR_SIZE) ) {
		ped_exception_throw (
			PED_EXCEPTION_ERROR,
			PED_EXCEPTION_CANCEL,
	...
	if (PED_BE32_TO_CPU(jh->jhdr_size) != PED_SECTOR_SIZE) {
		ped_exception_throw (
			PED_EXCEPTION_ERROR,
			PED_EXCEPTION_CANCEL,
	...
	if (jh->start == jh->size)
		jh->start = PED_CPU_TO_BE64(PED_SECTOR_SIZE);
	if (jh->end   == jh->size)
		jh->start = PED_CPU_TO_BE64(PED_SECTOR_SIZE);
