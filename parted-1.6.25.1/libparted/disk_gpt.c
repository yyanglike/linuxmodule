/*
    ...
    Portions Copyright (C) 2001, 2002, 2003 Free Software Foundation, Inc.
...
gpt_probe (PedDevice * dev)
{
	...
	if (ped_device_read(dev, &gpt, 1, 1)) {
		if (gpt.Signature == PED_CPU_TO_LE64(GPT_HEADER_SIGNATURE))
			gpt_sig_found = 1;
	}
	if (ped_device_read(dev, &gpt, dev->length - 1, 1)) {
		if (gpt.Signature == PED_CPU_TO_LE64(GPT_HEADER_SIGNATURE))
			gpt_sig_found = 1;
	}
	if (!gpt_sig_found)
	...
	if (ped_device_read(dev, &legacy_mbr, 0, 1)) {
...
{
	LegacyMBR_t pmbr;
	GuidPartitionTableHeader_t old_gpt;
	GuidPartitionTableHeader_t gpt;
	...
	if (!ped_device_read(dev, &old_gpt,
	...
	if (!ped_device_write(dev, &gpt,
			      GPT_PRIMARY_HEADER_LBA, GPT_HEADER_SECTORS))
		return 0;
	if (!ped_device_write(dev, &gpt, dev->length - GPT_HEADER_SECTORS,
			      GPT_HEADER_SECTORS))
		return 0;
...
static PedDisk *
gpt_alloc (PedDevice * dev)
{
	PedDisk* disk;
	GPTDiskData *gpt_disk_data;
	PedSector data_start, data_end;

	disk = _ped_disk_alloc (dev, &gpt_disk_type);
	if (!disk)
		goto error;
...
static int
_header_is_valid (GuidPartitionTableHeader_t* gpt)
{
...
static int
_read_header (PedDevice* dev, GuidPartitionTableHeader_t* gpt, PedSector sector)
{
	PED_ASSERT (dev != NULL, return 0);

	if (!ped_device_read (dev, gpt, sector,
			      sizeof (GuidPartitionTableHeader_t) / 512))
		return 0;
	return _header_is_valid (gpt);
}
		  ...
		  GuidPartitionTableHeader_t* gpt)
{
	...
	gpt->HeaderSize = PED_CPU_TO_LE32 (92); /* per 1.00 spec */
	gpt->HeaderCRC32 = 0;
	gpt->Reserved1 = 0;

	if (alternate) {
		PedSector ptes_size = gpt_disk_data->entry_count
				      * sizeof (GuidPartitionEntry_t) / 512;
	...
	gpt->HeaderCRC32 = PED_CPU_TO_LE32 (efi_crc32 (gpt, 92));
}
...
gpt_write(PedDisk * disk)
{
	...
	if (!ped_device_write (disk->dev, &gpt, 1, 1))
		goto error_free_ptes;
	if (!ped_device_write (disk->dev, ptes, 2, ptes_size / 512))
		goto error_free_ptes;

	/* Write Alternate PTH & PTEs */
	_generate_header (disk, 1, ptes_crc, &gpt);
	if (!ped_device_write (disk->dev, &gpt, disk->dev->length - 1, 1))
		goto error_free_ptes;
	if (!ped_device_write (disk->dev, ptes,
			       disk->dev->length - 2 - ptes_size / 512,
			       ptes_size / 512))
		goto error_free_ptes;
...
gpt_alloc_metadata (PedDisk * disk)
{
	...
	gptlength = ped_div_round_up (sizeof (GuidPartitionTableHeader_t), 512);
